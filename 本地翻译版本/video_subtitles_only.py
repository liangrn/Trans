...
def process_single_video(input_video_path, target_language, output_video_path):
    ...
    try:
        ...
        try:
            ...
            segments, info = model.transcribe(input_video_path, beam_size=5, best_of=5)
        ...
        traceback.print_exc()
        return False
def batch_process_videos(input_dir, output_dir, target_language, video_extension=".mp4", auto_merge=False, merged_filename="merged_output.mp4"):
    """批量处理视频（可选自动合并）"""
    print(f"\n--- 开始批量处理 ---")
    ...
    for input_path in video_files:
        ...
        print(f"\n处理: {filename}")
        if process_single_video(input_path, target_language, output_path):
            success_count += 1
