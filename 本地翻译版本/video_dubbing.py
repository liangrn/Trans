...
# 确保能导入同目录下的 local_translator
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
from local_translator import translate_text  # 直接导入替换函数
...
def translate_text(text, target_lang):
    """翻译文本 - 本地 NLLB 版 (覆盖原 Google 版)"""
    # 直接调用 local_translator 模块中的实现
    from local_translator import translate_text as local_translate
    try:
        import re
        # 检测是否包含中文字符（繁体/简体）
        ...
        return local_translate(text, target_lang)
    except Exception as e:
        print(f"  - 翻译失败: {e}")
        return text  # 回退到原文
...
def process_single_video(input_video_path, target_language, selected_voice_key, output_video_path, max_speed_factor, min_speed_factor, available_voices):
    """处理单个视频的函数 - 修复资源泄漏版"""
    import gc
    import numpy as np
    ...
    try:
        ...
        for i, seg_data in enumerate(translated_segments_data):
            print(f"\n--- 处理片段 {i+1}/{len(translated_segments_data)} ---")
            print(f"    原时间: [{seg_data['start']:.2f}s -> {seg_data['end']:.2f}s], 时长: {seg_data['original_duration']:.2f}s")
            print(f"    原文: {seg_data['original_text'][:50]}...")
            print(f"    翻译: {seg_data['translated_text'][:50]}...")
            
            # 5.1 生成原始TTS音频
            tmp_orig = tempfile.NamedTemporaryFile(delete=False, suffix='.wav')
            temp_tts_file = tmp_orig.name
            tmp_orig.close()
            temp_files_to_cleanup.append(temp_tts_file)
            
            try:
                synthesize_speech_coqui_single(tts_model, tts_speaker_idx,
            ...
            try:
                ...
                print(f"    TTS原时长: {os.path.getsize(temp_tts_file)} bytes")
            ...
            # 及时清理当前片段的临时TTS文件（保留调整后的文件用于合成）
            if os.path.exists(temp_tts_file):
                try:
                    os.remove(temp_tts_file)
                    if temp_tts_file in temp_files_to_cleanup:
                        temp_files_to_cleanup.remove(temp_tts_file)
                except:
                    pass
...
def batch_process_videos(input_dir, output_dir, target_language, selected_voice_key, max_speed_factor, min_speed_factor, available_voices, video_extension=".mp4"):
    """批量处理指定目录下的所有视频文件"""
    print(f"\n--- 开始批量处理视频 ---")
